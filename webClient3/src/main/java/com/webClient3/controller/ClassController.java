...
public class ClassController {
    ...
    public ModelAndView manageFile(@Valid @ModelAttribute("report") ReportError report, HttpSession session) {
        ...
        if (listEmail == null || listEmail.isEmpty()) {
            modelAndView.addObject("error", "File excel không có sinh viên nào!");
        } else {
            ...
            if (report.getErrorCode() == 200) {
                if (report.getDescription().equalsIgnoreCase("0-")) {
                    ...
                    modelAndView.addObject("error", "Không có tài kho?n nào h?p l?!");

                } else {
                    String[] infoReport = report.getDescription().split("-");
                    modelAndView.addObject("error", infoReport[0] + " tài kho?n không h?p l? ? c? dòng: " + infoReport[1]);

                }
            } else {
                modelAndView.addObject("error", "Thêm sinh viên theo file th?t b?i!");
            }
        }
...
//			e.printStackTrace();
//			modelAndView.addObject("error", "Táº¡o tÃ i khoáº£n khÃ´ng thÃ nh cÃ´ng!");
//			this.prepareForCreateStudentClassView(modelAndView);
//			return modelAndView;
//		}
    }
